...
/**
 * Merchant.country feeds assertResourceCountryScope, which compares it exactly
 * against an admin's ISO-3166-1 alpha-2 country scope. Free text like "Ghana"
 * can never match "GH", so a country-scoped admin is denied access to that
 * merchant. Existing rows hold free text, so map them onto a code where we can
 * and otherwise leave the field empty for the provider to re-pick.
 */
...
export function PaymentSettingsForm({ showBackButton = false }: { showBackButton?: boolean }) {
  ...
  async function handleSave(event: React.FormEvent) {
    ...
    try {
      ...
      showToast("Payment & payout details updated successfully.");
    } catch (error) {
        ...
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <OverviewCard
              ...
              detail="Stored on merchant profile."
              icon={<Wallet />}
            ...
            <form onSubmit={handleSave} className="mt-4 space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Select
                  ...
                  options={ALL_COUNTRIES.map((c) => ({ value: c.code, label: `${c.flag} ${c.name}` }))}
                />
              </div>
              ...
              <Card className="border border-slate-100 bg-slate-50/60" padding="sm">
                ...
                <div className="grid gap-2 sm:grid-cols-3">
                  {([
                    ...
                    >
                      <Icon className={cn("h-5 w-5 shrink-0", payoutMethod === value ? "text-emerald-700" : "text-slate-400")} />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-slate-800 truncate">{label}</p>
                        <p className="text-[10px] text-slate-400 truncate">{sub}</p>
                      </div>
                      {payoutMethod === value && <span className="ml-auto h-2 w-2 shrink-0 rounded-full bg-emerald-600" />}
                    </button>
                  ))}
                </div>
              </Card>
              ...
              {showStripeSection && (
                <div className="space-y-4">
                  <SectionNotice
                    ...
                    message="Connect Stripe to receive direct payouts, or refresh the link if onboarding expired."
                    tone="bg-slate-50 border-slate-100 text-slate-700"
                  />
                  ...
                  {profile?.stripeConnectAccountId ? (
                    ...
                    <SectionNotice
                      ...
                      message="Start Stripe onboarding to create your account and link details."
              ...
              {payoutMethod === "manual" && (
                <SectionNotice
                  ...
                  message="No payout account details are required for manual payout settlement."
                  tone="bg-slate-50 border-slate-100 text-slate-700"
                />
              )}
